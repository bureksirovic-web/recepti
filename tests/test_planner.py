...
class TestMealPlanner:
    ...
    def test_generate_weekly_plan_recipe_ids(self, sample_recipes):
        ...
        for day_plan in plans.values():
            ...
            assert (
                day_plan.breakfast_id is not None
                or day_plan.lunch_id is not None
                or day_plan.dinner_id is not None
            )
    ...
    def test_no_repeat_within_week(self, sample_recipes):
        ...
        for mp in plans.values():
            day_ids = [
                rid for rid in [mp.breakfast_id, mp.lunch_id, mp.dinner_id] if rid is not None
            ]
